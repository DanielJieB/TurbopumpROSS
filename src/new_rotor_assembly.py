...
ShaftSection(L=3.47282250, odl=0.4);

# LABY SECTION
PartitionedSection(L=0.9089, odl=(0.63 + 0.548)/2, partitions=2)
...
# simple sleeve
OverlappingSection(L=0.84970724, start=0.65269270, odl=0.669, idl=0.4);

OverlappingSection(L=0.93448861, start=1.50245567, odl=0.54468000, idl=0.4);
...
def Mark(object: any, position_inch: float):
    position_m = Q_(position_inch, 'in').to('m').m;
...
Mark(kero_inducer, 2.68662598425 + 0.05);
...
Mark(kero_impeller, 3.18677735 + 0.05);
...
Mark(spacer_lox, 5.88550079 - 0.5611);
...
Mark(lox_impeller, 6.30276813 - 0.5611);
...
Mark(lox_inducer, 6.832196456693 - 0.5611);
...
Mark(retaining_nut, 7.358319685039 - 0.5611);
...
def IsotropicBearing(bearing: rs.BallBearingElement) -> rs.BearingElement:
    ...
    #k = (kxx + kyy) / 2;
    #c = (cxx + cyy) / 2;
    return rs.BearingElement(n=0, kxx=kyy, cxx=cyy, tag=bearing.tag);
...
Mark(lox_bearing1, 4.57855039);

Mark(lox_bearing2, 4.97225118);
